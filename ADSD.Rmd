...
HCvsSD$Pvals_FDR05 = M[1:54,]
HCvsAD$Pvals_FDR05 = M[54:108,]
...
result = array(character(nrow(edges)*2), c(nrow(edges),2))
for (i in 1:nrow(edges)) {
  result[i,] = c(as.character(rois$AAL_Name[map$aal[match(edges[i,1], map$crad)]]),
                 as.character(rois$AAL_Name[map$aal[match(edges[i,2], map$crad)]]))
}
print(result)
```
...
## Mean
```{r Mean}
x=mHC[HCvsAD$Pvals_FDR05 < 0.05]
x[!is.na(x)]
x=mAD[HCvsAD$Pvals_FDR05 < 0.05]
x[!is.na(x)]
```