...
HCvsSD$perm.Svals_FDR05[HCvsSD$perm.Pvals_FDR05 >= 0.07 | is.na(HCvsSD$perm.Pvals_FDR05)] = 0
HCvsAD$perm.Svals_FDR05[HCvsAD$perm.Pvals_FDR05 >= 0.07 | is.na(HCvsAD$perm.Pvals_FDR05)] = 0
...
for (i in 1:nrow(edgesSD)) {
  ...
  d=data.frame(z=c(zSD[,i], zHC[,i]), group=as.factor(c(rep("SD", 23), rep("HC", 17))))
  
  p[[i]]= ggplot(d, aes(x=group, y=z)) + geom_violin() + geom_boxplot(width=0.2) +
    geom_point(shape=1, color="gray20", size=1, position = position_jitter(width = 0.2, height = 0.0002)) + 
    ggtitle(sprintf('%d %s\n%s', i, resultSD[i,1], resultSD[i,2])) + ylim(c(0, 0.8)) +
...
for (i in 1:nrow(edgesAD)) {
  ...
  d=data.frame(z=c(zAD[,i], zHC[,i]), group=as.factor(c(rep("AD", 16), rep("HC", 17))))
  
  p[[i]]= ggplot(d, aes(x=group, y=z)) + geom_violin() + geom_boxplot(width=0.2) +
    geom_point(shape=1, color="gray20", size=1, position = position_jitter(width = 0.2, height = 0.0002)) + 
    ggtitle(sprintf('%s\n%s', resultAD[i,1], resultAD[i,2])) + ylim(c(0, 0.8)) +
    theme(plot.title = element_text(size=10, face = "plain"))
}
...
idx = which(HCvsSD$perm.Pvals_FDR05 < 0.07, arr.ind = T)
edgesSD = array(tmpidx[idx], dim=c(nrow(idx),2))
print(edgesSD)
...
idx = which(HCvsAD$perm.Pvals_FDR05 < 0.07, arr.ind = T)
edgesAD = array(tmpidx[idx], dim=c(nrow(idx),2))
print(edgesAD)
...
## Z-scores SD
```{r Z-scores SD perm, warning=FALSE, fig.height=8, fig.width=8}
zSD = array(NA, dim=c(sum(infos$isSD & infos$included), nrow(edgesSD)))
zHC = array(NA, dim=c(sum(infos$isHC & infos$included), nrow(edgesSD)))
...
## Z-scores AD
```{r Z-scores AD perm, warning=FALSE, fig.height=4, fig.width=4.5}
zAD = array(NA, dim=c(sum(infos$isAD & infos$included), nrow(edgesAD)))
zHC = array(NA, dim=c(sum(infos$isHC & infos$included), nrow(edgesAD)))
...
plot_grid(p[[1]], p[[2]], p[[3]], p[[4]], p[[5]], ncol = 3, nrow = 2)
```
