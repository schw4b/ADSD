...
for (i in 1:N_ROIS) {
  for (j in 1:N_ROIS) {
    if (i != j ) {
      y = Z[i,j,]
      result = summary(aov(y[infos$included] ~ group[infos$included]))
      result = result[[1]]
...
### Significant FC
Label position with sign. group difference.
```{r Position of labels}
...
d = data.frame(R = R[3,1,], Z = Z[3,1,], group=group)
